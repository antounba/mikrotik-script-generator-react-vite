...
// 1. GENERATE BINDING (TAMBAH)
export const generateBindingScript = (bindings, options = { mode: 'hotspot' }) => {
    let script = "";
    const parents = {};
...
// GENERATE EDIT MAC ADDRESS BINDING
export const generateEditBindingScript = (editData) => {
    let script = "";
    editData.forEach((e) => {
        const ip = e.ip;
...
// GENERATE HAPUS BINDING
export const generateHapusBindingScript = (hapusData) => {
    let script = "";
    hapusData.forEach((h) => {
        const ip = h.ip;
        if (ip) {
